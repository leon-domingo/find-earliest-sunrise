...
const ConcurrentTasksManager = require('./concurrent-tasks-manager');

function findEarliestSunrise({ apiURL, maxConcurrentFetch, numberOfPoints, date }) {
  ...
  const tasksManager = new ConcurrentTasksManager({
    ...
    onEnd() {
      if (this.getSuccessfulTasks().length > 0) {
        const [latitude, longitude, date] = taskWithEarliestSunrise.params.slice(1);
        ...
        const parsedSunrise = parse(sunrise, timeFormat, parsedDate);
        let parsedSunset = parse(sunset, timeFormat, parsedDate);
  ...
  randomCoords.forEach(({ latitude, longitude }) => {
    tasksManager.addSingleTask({
      ...
      // asyncFunc: getSunriseSunsetDataMock(0, 1000),
      params: [apiURL, latitude, longitude, date],
    });
  });
