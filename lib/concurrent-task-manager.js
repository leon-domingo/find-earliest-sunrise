...
ConcurrentTasksManager.prototype.runTasks = function(runForever = false) {
  ...
  const pollingInterval = setInterval(() => {
    if (this.getFinishedTasks().length === this.tasks.length && !runForever) {
      ...
      const elapsedtimePerTaskOnAverage = elapsedTime / this.getSuccessfulTasks().length;
      this.consoleLog(`Elapsed time ${elapsedTime}ms to complete ${this.getSuccessfulTasks().length} successful task/s out of ${this.tasks.length}. ${elapsedtimePerTaskOnAverage.toFixed(2)}ms for each task on average`);
